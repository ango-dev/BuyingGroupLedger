...
# `import models.order`, `import ledger.sync`, etc. the same way main.py does.
...
def _block_real_alerts(monkeypatch):
    ...
    production code paths (main.py, every scraper, ledger/sync.py), and remembering to mock it
...
def _isolate_ledger_db(tmp_path_factory, monkeypatch):
    ...
    An un-fixtured call into anything that opens the ledger -- ledger.sync._get_worksheet,
    the dashboard's writer, the read-only opener -- lands on the SQLite file, and every settings object names it by the RELATIVE path
