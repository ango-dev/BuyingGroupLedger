...
- **Audit** (`/audit`, `web/audit_view.py`; its description, tiles and checks panel come first and scroll away, after which the heading and the filter bar stick to the top; the four tiles filter the rows to the checks they count): every check of `scripts.audit_ledger` run against the
