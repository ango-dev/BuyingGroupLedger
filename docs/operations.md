...
- **Audit** (`/audit`, `web/audit_view.py`; its description, tiles and checks panel scroll away with the rows while the filter bar stays pinned, as on Orders; the four tiles filter the rows to the checks they count): every check of `scripts.audit_ledger` run against the
