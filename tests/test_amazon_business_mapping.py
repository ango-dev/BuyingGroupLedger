...
def _cancelled_order_page(oid: str, extra_shipments: list[str] | None = None) -> str:
    ...
    html = html.replace("<div>Payment method Prime Business Card ending in 1234 5% back</div>", "")
...
def test_the_promo_rate_is_read_from_the_card_rows_feature_detail():
    ...
    html = _details(oid, "September 17, 2026",
        ...
        .replace("<div>Payment method Prime Business Card ending in 1234 5% back</div>", _widget(_widget_card_with_feature(
...
def test_a_consumer_cards_earn_detail_carries_no_promo():
    ...
    html = _details(oid, "September 17, 2026",
                    [_shipment(oid, 0, "Arriving tomorrow", [_item("PS5", "$599.00", qty=2)])]) \
        .replace("<div>Payment method Prime Business Card ending in 1234 5% back</div>", _widget(_widget_card_with_feature("$1,071.98 (Earns 5% back)", "Prime Visa", "4345")))
...
def test_a_cash_back_rows_detail_never_reads_as_a_promo_or_an_unknown_tender():
    ...
    html = _details(oid, "September 17, 2026",
                    [_shipment(oid, 0, "Arriving tomorrow", [_item("PS5", "$599.00", qty=2)])]) \
        .replace("<div>Payment method Prime Business Card ending in 1234 5% back</div>", _widget(_WIDGET_CASH_BACK_APPLIED_ROW,
...
def test_the_widgets_applied_amount_stands_in_only_when_the_summary_line_is_missing():
    ...
    widget = _widget(_WIDGET_CASH_BACK_APPLIED_ROW,
                     _widget_card_with_feature("$1,071.98 (Earns 5% back)", "Prime Visa", "4345"))
    base = _details(oid, "September 17, 2026", shipments, subtotal="$1,198.00").replace("<div>Payment method Prime Business Card ending in 1234 5% back</div>", widget)
