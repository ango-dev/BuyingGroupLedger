...
def _cash_back_consumed(summary_el) -> float | None:
    ...
    -$89.10"), so the ledger held $89.10 PER ORDER instead of $89.10 split across them. The line is
...
def missing_card_reason(order_details_html: str) -> str | None:
    ...
    run: the parse "succeeded", so no dossier was
    written and nothing alerted until the ledger audit. The API client calls this after a successful
    build whose rows all carry a blank card and, given a reason, files a dossier problem WITH the
    page attached while still RECORDING the rows — one unreadable field must alert, not drop
    reimbursement money (the blank fills itself once the parser is fixed).
...
def rewards_used_amount(summary_el, region, points_used: float | None, order_id: str = "") -> float | None:
    ...
    year end outside the ledger, so the order keeps its FULL cost here and only the cashback basis
...
def _sales_tax_amount(summary_el) -> float | None:
    ...
    overwrites a figure on the ledger.
    """
    if summary_el is None:
        return None
...
def _collapse_same_package_cards(rows: list[OrderItem], subtotal: float | None,
    ...
    """Two shipment cards of ONE parse carrying the SAME shipmentId are one package — keep one.

    The structural twin of _reconcile_against_subtotal below (which runs after this and stays as the
    backstop). On the re-labelled order of 2026-08-22 the ledger's two duplicate rows both carried the
    NEW shipmentId in their Tracking Link, which is what the double render most likely looked like:
    the superseded card and its replacement pointing at one shipment object. A healthy page never
    repeats a shipmentId across cards (76 captured tokens, one per package), so this only ever fires
    on a duplicate — and unlike the subtotal guard it needs no `Item(s) Subtotal` line to do so.
...
def build_order_items(
    ...
    # visible on the ledger. The toggle keeps its old name and meaning — netting off means the amount
