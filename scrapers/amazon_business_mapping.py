...
def _cash_back_consumed(summary_el) -> float | None:
    ...
    the summary line showed the
    whole BALANCE ("Prime for Young Adults cash back: -$89.10") on a $35.93 order whose Grand
    Total read $0.00 — the line is the balance applied at checkout, not the amount spent. What the
    order consumed is what the card was not charged: subtotal + shipping + tax − gift card −
    Grand Total. The cap only applies when every piece parses (the synthetic-fixture summaries
    usually omit the subtotal, and a healthy line equals the cap anyway — proven on the
    09-05/09-07 orders)."""
